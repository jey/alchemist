...
# note that currently each alchemist worker gets 8 cores (so has 4 workers/driver per node)
#
# Example use: launch 15 Spark nodes (14 executors) and 5 Alchemist nodes (4 workers)
# salloc -N 20 -t 15 --qos=interactive -C haswell
# source setup/cori-start-alchemist.sh 5
...
# note that the release build of Elemental (currently used in the bootstrap script) uses one core per mpi process, so 
# let alchemist launch as many processes as there are cores on its nodes
# TODO: look into HybridRelease, which uses OpenMP + MPI
# BE CAREFUL: SRUN NEEDS AN ABSOLUTE PATH TO THE EXECUTABLE
srun -N ${ALCHEMISTNODECOUNT} -n $((${ALCHEMISTNODECOUNT}*4)) --cpus-per-task=8 -w $SPARK_WORKER_DIR/hosts.alchemist ./core/target/alchemist &
